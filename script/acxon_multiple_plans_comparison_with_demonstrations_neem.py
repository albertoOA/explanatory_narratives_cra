...
if __name__ == '__main__': 
    ...
    results_dict = {"Pair ID": list(), "Dataset" : list(), "Specificity" : list(), \
                    "Memory" : list(), "Triples" : list(), \
                    "Words": list(), "Readability score": list()}
    
    explanations_dict = {"Pair ID": list(), "Explanation" : list()}
    ...
    narratives_file = rospack.get_path('explanatory_narratives_cra') + \
      "/txt/acxon_based/"+planning_domain+"/generated_c_narratives_multiple_plans_comparison_with_specificity_" + str(specificity) + "_problem_" + dataset_name + ".txt"
    evaluation_results_file = rospack.get_path('explanatory_narratives_cra') + \
      "/csv/acxon_based/" + planning_domain + "/problem_" + dataset_name + "_with_specificity_" + str(specificity) + ".csv"
    """
    narratives_file = rospack.get_path('explanatory_narratives_cra') + \
      "/txt/acxon_based" + "/generated_c_narratives_multiple_plans_comparison_with_specificity_" + str(specificity) + "_dataset_" + dataset_name + ".txt"
    narratives_csv_file = rospack.get_path('explanatory_narratives_cra') + \
      "/csv/acxon_based" + "/generated_c_narratives_multiple_plans_comparison_with_specificity_" + str(specificity) + "_dataset_" + dataset_name + ".csv"
    evaluation_results_file = rospack.get_path('explanatory_narratives_cra') + \
      "/csv/acxon_based" + "/dataset_" + dataset_name + "_with_specificity_" + str(specificity) + ".csv"
    ...
    for pair_to_compare_id, tuples_of_the_pair in tuples_dict.items():
      ...
      combined_c_narrative_mod = combined_c_narrative.replace("has worse quality value", "has a higher value")
      ...
      explanations_dict['Pair ID'].append(pair_to_compare_id)
      explanations_dict['Explanation'].append(combined_c_narrative_mod)
      ...
      results_dict['Pair ID'].append(pair_to_compare_id)
      results_dict['Dataset'].append(dataset_name)
      results_dict['Specificity'].append(specificity)
      results_dict['Memory'].append(test_object.get_memory_usage()['vmpeak'])
      results_dict['Triples'].append(triples_count)
      results_dict['Words'].append(words_count)
      results_dict['Readability score'].append(read_score)

    f.close()

    df = pd.DataFrame.from_dict(explanations_dict, orient='columns')
    df.to_csv(narratives_csv_file, index=False) # data frame

    df = pd.DataFrame.from_dict(results_dict, orient='columns')
    df.to_csv(evaluation_results_file, index=False) # data frame

    print("[acxon_multiple_plans_comparison_with_neem.py] Narratives have been properly generated, check the 'txt'.")
    
    print("\n ·· TEST ·····\n Memory usage (peak): ", results_dict["Memory"], " MB") # 'vmpeak' 'vmsize' 'vmlck' 'vmpin' 'vmhwm' 'vmrss' 'vmdata' 'vmstk' 'vmexe' 'vmlib' 'vmpte' 'vmswap'
